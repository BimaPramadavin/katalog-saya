...
export default function AdminPage() {
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-6 text-white font-sans">
        <div className="max-w-sm w-full bg-zinc-900 border border-zinc-800 p-10 rounded-[2.5rem] shadow-2xl text-center">
          <h1 className="text-2xl font-black mb-2 italic tracking-tighter uppercase leading-none">Admin Gate</h1>
          ...
          <form onSubmit={(e) => {
            ...
            <button className="w-full bg-white text-black font-black py-4 rounded-2xl uppercase text-[11px] tracking-widest hover:bg-zinc-200 active:scale-95 transition-all">Login</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9] p-4 md:p-12 text-black font-sans">
      <div className="max-w-6xl mx-auto">
        
        <header className="flex justify-between items-center mb-12">
          <div>
            <h1 className="text-3xl font-black italic tracking-tighter uppercase leading-none">Management</h1>
            <p className="text-zinc-400 text-[10px] font-black uppercase tracking-[0.3em] mt-1">Inventory Dashboard</p>
          </div>
          <button 
            onClick={() => { 
            ...
            className="text-[10px] font-black border border-zinc-200 px-6 py-3 rounded-full uppercase tracking-tighter hover:bg-red-500 hover:text-white hover:border-red-500 transition-all active:scale-95 shadow-sm"
          >
            Sign Out
          </button>
        </header>
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2">
            <div className={`p-8 rounded-[2.5rem] shadow-2xl transition-all duration-300 border sticky top-10 
              ${editId ? 'bg-blue-50/50 border-blue-200 shadow-blue-100' : 'bg-white border-zinc-100 shadow-zinc-200/50'}`}>
              
              <h2 className="text-[10px] font-black uppercase tracking-[0.2em] mb-8 text-zinc-400">
                {editId ? "Mode Edit Aktif" : "Data Produk Baru"}
              </h2>
              ...
              <form onSubmit={handleSimpan} className="space-y-5">
                ...
                <div className="flex flex-col gap-3 pt-4">
                    <button 
                      type="submit" 
                      disabled={loading} 
                      className={`w-full p-5 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-lg transition-all duration-300 transform 
                        ${editId 
                          ? 'bg-blue-600 text-white shadow-blue-200 hover:shadow-blue-400 hover:-translate-y-1' 
                          : 'bg-black text-white shadow-zinc-300 hover:shadow-zinc-500 hover:-translate-y-1'
                        } 
                        active:scale-95 disabled:bg-zinc-300 disabled:shadow-none disabled:translate-y-0`}
                    >
                      {loading ? "PROCESSING..." : editId ? "UPDATE CHANGES" : "PUSH TO CATALOG"}
                    </button>
                    {editId && (
                        <button onClick={batalkanEdit} type="button" className="w-full p-2 text-[9px] font-black uppercase text-zinc-400 hover:text-black transition-colors">Batalkan Edit</button>
                    )}
                </div>
              </form>
            </div>
          ...
          <div className="lg:col-span-3">
            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] mb-8 text-zinc-400">Inventory Status ({produk.length})</h2>
            <div className="grid grid-cols-1 gap-4">
              {produk.map(p => (
                <div key={p.id} className="bg-white p-4 rounded-[2rem] flex items-center gap-6 border border-zinc-100 hover:shadow-xl transition-all duration-300 group">
                  <img src={p.image_url} className="w-20 h-20 object-cover rounded-2xl shadow-inner bg-zinc-50" />
                  <div className="flex-1">
                    <h3 className="font-black text-sm uppercase text-zinc-800 leading-tight tracking-tight">{p.nama}</h3>
                    <p className="text-blue-600 font-black text-xs mt-1">Rp {p.harga?.toLocaleString()}</p>
                    <div className="flex gap-2 mt-2">
                      <span className="text-[8px] font-black px-2 py-1 bg-zinc-50 rounded text-zinc-400 uppercase tracking-tighter">SZ: {p.size || '-'}</span>
                      <span className="text-[8px] font-black px-2 py-1 bg-zinc-50 rounded text-zinc-400 uppercase tracking-tighter">CD: {p.kondisi || '-'}</span>
                    </div>
                  </div>
                  
                  <div className="flex gap-2 pr-2">
                    <button onClick={() => mulaiEdit(p)} className="p-4 bg-zinc-50 rounded-2xl text-zinc-400 hover:bg-blue-600 hover:text-white transition-all active:scale-90 shadow-sm">
                    ...
                    <button onClick={() => handleHapus(p.id)} className="p-4 bg-zinc-50 rounded-2xl text-zinc-400 hover:bg-red-600 hover:text-white transition-all active:scale-90 shadow-sm">
